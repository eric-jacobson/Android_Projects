...
import com.example.talker.R.id.scan_button
...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleBTDevice(intent: Intent) {
        ...
        if (BluetoothDevice.ACTION_FOUND == action) {
            ...
            if (deviceName.length > 3) { //for now, looking for MSU prefix
                ...
                if (prefix == "Gal" || prefix == "BLU") {//This is the server
    ...
    fun echoMsg(msg: String, action: String) {
        ...
        if(action.compareTo("a") == 18){shake()}
        else if(action.compareTo("a") == 5){flash()}
        else if(action.compareTo("a") == 1){beep()}
        else{mTextarea!!.append("Unrecognized message: '" + action + "'\n" + action.compareTo("a"))}
        ...
        private fun manageConnectedSocket(socket: BluetoothSocket) {
            ...
            try {
                flashButton?.setOnClickListener {
                    ...
                    out.write(theMessage.toByteArray())
                }
                val beepButton = beepButton
                beepButton?.setOnClickListener {
                    theMessage = "beep"
                    mTextarea!!.append("Message has been updated to beep")
                    out.write(theMessage.toByteArray())
                }
                val shakeButton = shakeButton
